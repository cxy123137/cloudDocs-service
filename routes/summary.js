...
summaryRouter.post('/generate', async (req, res) => {
  try {
    ...
    console.log('generateSummary收到docId:', docId, '类型:', typeof docId);
    console.log('generateSummary收到userId:', userId, '类型:', typeof userId);
    
    if (!docId) {
      return res.status(400).json({ error: '缺少docId参数' });
    }
    
    // 验证docId格式
    if (typeof docId !== 'string' || docId.length !== 24 || !/^[a-fA-F0-9]{24}$/.test(docId)) {
      return res.status(400).json({ error: 'docId格式错误，必须为24位hex字符串' });
    }
